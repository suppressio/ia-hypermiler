...
│   └── generate-icons.js   ← genera l'icona "gauge" (build/icon.png, renderer/assets/app-icon.png, renderer/assets/tray-icon.png); one-off, non fa parte di `npm run build`
├── build/
│   └── icon.png            ← sorgente icona per electron-builder (genera .ico/.icns automaticamente, NON copiata in dist/)
...
- `npm test` — build + `cd dist && node --test` (nota: NON `node --test dist` dalla root — vedi CLAUDE.md, Stato avanzamento, "Fix CI: `node --test dist` fallisce su Node 22"). I test unitari (`budget.test.ts`, `services/*.test.ts`) girano sempre, mockano `fetch` e non toccano la rete.
...
- **Ogni job:** `npm ci` → `npm test` (build + test unitari, fallisce la CI se i test non passano) → `npx electron-builder --<mac|win|linux> --publish=always`.
- **Output:** i pacchetti (`.dmg`/`.exe`/`.AppImage`/`.deb`) vengono **allegati direttamente alla GitHub Release del tag corrente** (electron-builder `--publish=always`, con permesso di job `contents: write` sul `GITHUB_TOKEN` di default — se il tag non ha ancora una Release, electron-builder ne crea una come bozza) **e** caricati come artifact della run di Actions (retention 14 giorni), ridondante ma comodo per run lanciate da `workflow_dispatch` senza tag. Prima di questa modifica il workflow usava `--publish=never` (solo artifact, nessuna Release) — cambiato su richiesta esplicita dell'utente dopo aver notato che la Release creata a mano mostrava solo lo zip "Source code" generato automaticamente da GitHub, non i pacchetti compilati.
...
- **Verificato con run reali** (vedi righe "Stato avanzamento" sotto): mac e windows completano l'intera pipeline (build, test, packaging) con esito verde; linux idem dopo il fix dell'`author.email` e dell'icona di default. Il comportamento di `--publish=always` (allegare i pacchetti alla Release) non è ancora stato riconfermato dall'utente con una run successiva a questo cambio — da verificare pushando un nuovo tag.
...
| — | Primo run CI reale + fix: `node --test dist` fallisce su Node 22 | ✅ fatto | Prima release taggata dall'utente (`v0.1.0-beta`, dopo aver scoperto — vedi riga sopra — che il tag deve iniziare per `v` perché il workflow parta). Le 3 build (mac/linux/windows) sono fallite tutte allo stesso step "Build + test", in pochi secondi, subito dopo `npm ci` — sintomo di un problema nel comando, non specifico a un OS. Diagnosticato scaricando i metadati della run reale via API pubblica GitHub (`curl`/`WebFetch` hanno accesso di rete diretto da questo ambiente, a differenza di quanto indicato altrove in questo file per l'esecuzione dei test — verificato sul campo in questa sessione), log del job ottenuto dall'utente (il download dei log richiede permessi da admin del repo, non disponibili qui). Errore reale: `node --env-file-if-exists=.env.test --test dist` va in `MODULE_NOT_FOUND` su Node 22.23.1 (`Cannot find module '.../dist'`) mentre funziona su Node 20.19.1 (usato in locale) — il test runner nativo di Node gestisce diversamente un argomento posizionale "directory nuda" tra le due major version (Node 21+ ha introdotto il supporto ai glob per `--test`, cambiando l'interpretazione dei path passati esplicitamente); un glob esplicito (`--test "dist/**/*.test.js"`) risolverebbe su Node 22 ma non è supportato affatto su Node 20 (verificato: fallisce con "Could not find"), quindi non è un fix cross-version. **Fix adottato**: `cd dist && node --test` (nessun argomento di path esplicito, si usa lo scan automatico della directory corrente sulla pattern di default) — verificato funzionante identico su Node 20.19.1 in locale (58/58 test), e si affida allo stesso meccanismo di default-discovery documentato in modo stabile sia in Node 20 sia in Node 22 (non al codice path "argomento posizionale esplicito" che diverge tra le due versioni). Aggiustato di conseguenza anche il riferimento a `.env.test` (ora `../.env.test`, relativo alla nuova cwd `dist/` dopo il `cd`) — verificato che il file venga letto correttamente dalla nuova posizione con un test manuale prima di applicare il fix. Nota a margine: Node 20 ha raggiunto End-of-Life nell'aprile 2026 — l'ambiente locale usato in questa sessione (20.19.1) è quindi già fuori supporto; il fix scelto funziona comunque su entrambe le versioni, ma varrebbe la pena allineare anche l'ambiente locale a Node 22+ in futuro |
| — | Secondo run CI reale: fix `author.email` mancante + icona Linux di default | ✅ fatto | Col fix precedente, mac/windows sono passate interamente (confermato dall'utente). Linux è arrivato fino al packaging vero (AppImage generato) ma è fallito sul target `.deb`: electron-builder richiede un'email nel campo `author` di `package.json` (diventa il "Maintainer" del pacchetto Debian) — era `""`. Poiché il repo è pubblico, chiesto esplicitamente all'utente quale email mostrare invece di usare la sua email di lavoro vista nel contesto di sistema: scelta l'email privata `id+username@users.noreply.github.com` di GitHub (id numerico recuperato via API pubblica `api.github.com/users/suppressio`), MAI l'email reale. Trovato e corretto anche un problema collegato: il log mostrava "default Electron icon is used" durante il build dell'AppImage — causa: electron-builder genera l'icona Linux derivandola dal `.icns` di macOS, ma sulla CI ogni piattaforma builda a sé (il job Linux non esegue mai `--mac`), quindi su un runner Linux non c'è mai un `.icns` da cui derivare. Fix: `scripts/generate-icons.js` ora genera anche `build/icons/{16,24,32,48,64,96,128,256,512,1024}x{n}.png` (convenzione electron-builder, nessuna voce aggiuntiva in `package.json`) — icone ispezionate visivamente anche alla dimensione più piccola (16×16), ancora leggibili. Verificato: 58/58 test invariati |
...
| — | Rifiniture grafiche: disabilita Copilot su seat aziendale, icona gauge, pin neutro+glow, refresh manuale | ✅ fatto | Quattro richieste dopo la chiusura della ricerca Copilot: (1) checkbox "Account collegato/abilitato" di Copilot disabilitata automaticamente (con icona ⓘ esplicativa) solo quando `accountScope === 'organization'` — il piano personale resta pienamente utilizzabile; nuova `updateCopilotEnabledLock()` in `renderer/settings.ts`, nessuna modifica a `main.ts` (basta forzare `enabled=false` perché `isCopilotConnected()` già lo richieda). (2) Icona applicativa/tray "gauge" al posto del logo Electron di default: nessun tool di conversione immagini disponibile in questo ambiente (niente ImageMagick/Inkscape/Pillow/sharp/canvas) — creato `scripts/generate-icons.js`, un rasterizzatore vettoriale puro-Node con supersampling 4x per l'antialiasing + encoder PNG scritto a mano (`zlib.deflateSync` per IDAT, CRC32 table-based), zero dipendenze nuove. Genera `build/icon.png` (1024×1024, sorgente per la generazione automatica di .ico/.icns di electron-builder — aggiunto `"icon": "build/icon.png"` in `package.json`) e una copia in `renderer/assets/app-icon.png` (usata da `main/windows.ts` per l'icona di entrambe le finestre anche in `npm start` non pacchettizzato: `build/` non viene copiata in `dist/`, `renderer/assets/` sì, tramite `scripts/copy-assets.js` già esistente) più `renderer/assets/tray-icon.png` (64×64, già atteso da `main/tray.ts`, prima assente). Icone generate e ispezionate visivamente con il tool Read (supporta immagini) prima di procedere. (3) Pin "sempre in primo piano": l'emoji 📌 (colorata, non risponde a `color:` CSS) sostituita con un'icona SVG inline (`fill="currentColor"`); rimosso il riempimento pieno `background: var(--accent)` da acceso (giudicato troppo invadente) — ora da spento nessun effetto, da acceso solo `filter: drop-shadow(...)` statico (nessuna animazione, coerente con "Non aggiungere animazioni" di CLAUDE.md). (4) Nuovo pulsante "Aggiorna ora" (icona SVG doppia freccia circolare) accanto al pin, in `.titlebar-controls-left`: riusa il bridge già esistente `requestUsageRefresh()` (nessun nuovo canale IPC), disabilitato al click e riabilitato quando arriva il prossimo `onUsageUpdate`. Verificato: 58/58 test unitari invariati (nessuna logica testata cambia) — verifica visiva completa (`npm start`, icona di pacchetto via `npm run package`) non eseguibile in questo ambiente sandbox |
